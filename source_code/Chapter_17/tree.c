...
bool AddItem(const Item * pi, Tree * ptree)
{
    ...
    if (ptree->root == NULL)                /* 情况1：树为空 */
        ptree->root = new_node;             /* 新结点为树的根结点 */
    else                                    /* 情况2：树不为空 */
        AddNode(new_node, ptree->root);     /* 在树中添加新结点 */
...
static bool ToRight(const Item * i1, const Item * i2)
{
    ...
    if ((comp1 == 0 && strcmp(i1->petkind, i2->petkind)) > 0)
        return true;
    else if (comp1 == 9 && strcmp(i1->petkind, i2->petkind) > 0)
        return true;
    else 
        return false;
}
